...
public class Ficha
{
    ...
    public void RecogerObjeto()
    {
        ObjetosRecogidos++;
        Console.WriteLine(
            $"{Nombre} ha recogido un objeto! Total objetos recogidos: {ObjetosRecogidos}"
        );
        //Condicion de ganar
    ...
    public bool Mover(int nuevaFila, int nuevaColumna, Casilla[,] tablero)
    {
        ...
        if (turnosPasoFantasma > 0 && casillaDestino == Casilla.Obstaculo)
        {
            ...
            bool esBorde =
                nuevaFila == 0
                || nuevaFila == tablero.GetLength(0) - 1
                || nuevaColumna == 0
                || nuevaColumna == tablero.GetLength(1) - 1;

            if (!esBorde)
            {
                Console.WriteLine(
                    $"{Nombre} atraviesa un obstáculo debido a la habilidad Paso Fantasma."
                );
                Fila = nuevaFila;
    ...
    public bool PerderPuntos(int puntos)
    {
        ...
        else
        {
            Puntos -= puntos;
            if (Puntos < 0)
                Puntos = 0; // Asegurarse de que los puntos no sean negativos
            MostrarVida();
    ...
    public void UsarHabilidad(Casilla[,] tablero, Random random, List<Ficha> fichasSeleccionadas)
    {
        ...
        {
            Console.WriteLine(
                $"La habilidad {Habilidad} está en enfriamiento por {Cooldown} turnos más."
            );
            return;
        }
        ...
        switch (Habilidad)
        {
            ...
            case "Paso Fantasma":
                if (turnosPasoFantasma <= 0)
                {
                    ...
                    Console.WriteLine(
                        $"{Nombre} ha activado 'Paso Fantasma' y podrá atravesar obstáculos durante {turnosPasoFantasma} turnos."
                    );
                }
                else
                {
    ...
    private void TeletransportarAleatoriamente(
        Casilla[,] tablero,
        Random random,
        List<Ficha> fichasSeleccionadas
    )
    {
        int fila,
            columna;
    ...
    private void Curar()
    {
        Puntos += 2;
        if (Puntos > 5)
            Puntos = 5; // Asegura que los puntos no excedan 4
    ...
    {
        Console.WriteLine(
            $"{Nombre} Puede moverse 2 veces seguidas y luego otravez al presionar `Mover Ficha`"
        );
        Thread.Sleep(3000); // Pausa de 3 segundos
        ...
        {
            Console.WriteLine(
                $"Movimiento {i + 1} de 2: Usa las teclas de dirección para mover la ficha."
            );
            ...
            if (IsMovimientoValido(nuevaFila, nuevaColumna, tablero))
            {
                ...
                Console.WriteLine(
                    $"{Nombre} se ha movido a la posición ({nuevaFila}, {nuevaColumna})."
                );
                TableroDrawer.DibujarTablero(tablero, fichasSeleccionadas);
            }
            else
    ...
    public static bool IsMovimientoValido(int nuevaFila, int nuevaColumna, Casilla[,] tablero)
    {
        if (
            nuevaFila < 0
            || nuevaFila >= tablero.GetLength(0)
            || nuevaColumna < 0
            || nuevaColumna >= tablero.GetLength(1)
        )
        {
